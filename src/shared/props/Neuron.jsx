...
/**
 * Canonical Neuron Component
 * Derived from user's neuron.svg (viewBox: 0 0 260 235)
 * Center of soma / nucleus is at (113.085, 131.295)
 * Supports breaking in half with realistic jagged fracture & separation
 */
export const NEURON_SVG_PATH =
...
export const NEURON_TOP_CLIP =
  "M -60 -60 L 320 -60 L 320 128 L 205 137 L 165 124 L 135 136 L 113 120 L 95 138 L 65 127 L -60 130 Z";
export const NEURON_BOTTOM_CLIP =
  "M -60 130 L 65 127 L 95 138 L 113 120 L 135 136 L 165 124 L 205 137 L 320 128 L 320 300 L -60 300 Z";
export const NEURON_SEAM_PATH =
  "M -60 130 L 65 127 L 95 138 L 113 120 L 135 136 L 165 124 L 205 137 L 320 128";
...
export const Neuron = ({
  ...
  stroke = "#FFFFFF",
  strokeWidth = 1.2,
  filter = "url(#user-neuron-glow)",
  opacity = 1,
  showNucleus = true,
  nucleusColor = "#FFFFFF",
  breakProgress = 0, // 0 = whole, > 0 = broken in half
  breakCrackColor = "#EF4444",
  showCrackLine = true,
  crackGlow = true,
}) => {
  const clipTopId = `${id}-clip-top`;
  const clipBotId = `${id}-clip-bottom`;

  // Render the core contents of a half
      ...
      {showNucleus && (
        <>
          <ellipse
            ...
            opacity={0.85}
          />
          <circle cx={113.085} cy={131.295} r={5.5} fill="#EF4444" />
        </>
      )}
    </>
  );

  // If completely unbroken, render single intact group
  if (breakProgress <= 0) {
    return (
      <g transform={`scale(${scale})`} opacity={opacity}>
        <g transform="translate(-113.085, -131.295)" filter={filter}>
          {renderCellBody()}
        </g>
      </g>
    );
  ...
  // Broken in half: calculate offsets and rotation
  const topOffsetX = -breakProgress * 18;
  const topOffsetY = -breakProgress * 28;
  const topRot = -breakProgress * 15;

  const botOffsetX = breakProgress * 16;
  const botOffsetY = breakProgress * 26;
  const botRot = breakProgress * 14;

  return (
    <g transform={`scale(${scale})`} opacity={opacity}>
      <defs>
        <clipPath id={clipTopId}>
          <path d={NEURON_TOP_CLIP} />
        </clipPath>
        <clipPath id={clipBotId}>
          <path d={NEURON_BOTTOM_CLIP} />
        </clipPath>
      </defs>

      <g transform="translate(-113.085, -131.295)" filter={filter}>
        {/* Top Half */}
        <g transform={`translate(${topOffsetX}, ${topOffsetY}) rotate(${topRot}, 113.085, 131.295)`}>
        ...
        {/* Bottom Half */}
        <g transform={`translate(${botOffsetX}, ${botOffsetY}) rotate(${botRot}, 113.085, 131.295)`}>
          <g clipPath={`url(#${clipBotId})`}>
            {renderCellBody()}
          </g>
        </g>

        {/* Severed Jagged Crack Line along Fracture Seam */}
        {showCrackLine && (
          <g opacity={Math.min(1, breakProgress * 1.5)}>
            <path
              d={NEURON_SEAM_PATH}
              fill="none"
              stroke={breakCrackColor}
              strokeWidth={3.5}
              strokeLinecap="round"
              filter={crackGlow && filter && filter !== "none" ? "drop-shadow(0 0 6px #EF4444)" : undefined}
            />
            <path
              d={NEURON_SEAM_PATH}
              fill="none"
