...
export const JumpRope = ({
  ropePhase = "floor", // "floor", "pickup", "skipping", "held"
  skipAngle = 0,
  leftHand = { x: 70, y: 310 },
  rightHand = { x: 170, y: 310 },
  isBehind = false,
}) => {
  // Handle renderer (attached to hand)
  ...
  // --- STATE 1: RESTING ON THE FLOOR (Before pick up) ---
  if (ropePhase === "floor") {
    ...
    return (
      <g id="jump-rope-resting">
        ...
        {/* Rope cord resting in comic loop on floor */}
        <path
          d="M 65 506 Q 90 514 120 514 Q 150 514 175 506"
          fill="none"
          stroke="#000000"
          strokeWidth="6.5"
          strokeLinecap="round"
  ...
  if (ropePhase === "pickup") {
    ...
    return (
      <g id="jump-rope-pickup">
        <path
          d={`M ${leftHand.x} ${leftHand.y} Q ${midX} ${bottomY} ${rightHand.x} ${rightHand.y}`}
          fill="none"
          stroke="#000000"
          strokeWidth="6.5"
          strokeLinecap="round"
  ...
  // --- STATE 4: TRIUMPHANT HELD (Frames 318) ---
  if (ropePhase === "held") {
    if (isBehind) return null;
    return (
      <g id="jump-rope-held">
        <path
          d={`M ${leftHand.x} ${leftHand.y} Q ${midX} 460 ${rightHand.x} ${rightHand.y}`}
          fill="none"
          stroke="#000000"
          strokeWidth="6.5"
          strokeLinecap="round"
  ...
  // --- STATE 5: DROPPING TO FLOOR (Frames 318 - 338) ---
  if (ropePhase === "dropping") {
    if (isBehind) return null;
    return (
      <g id="jump-rope-dropping">
        {/* Shadow growing on floor as rope lands */}
        <ellipse
          cx="120"
          cy="514"
          rx="68"
        ...
        />
        <path
          d={`M ${leftHand.x} ${leftHand.y} Q ${midX} 514 ${rightHand.x} ${rightHand.y}`}
          fill="none"
          stroke="#000000"
          strokeWidth="6.5"
          strokeLinecap="round"
  ...
  // --- STATE 3: SKIPPING (Frames 228 - 318) ---
  // skipAngle: 0 to 2*PI
  // elev = -Math.cos(skipAngle): -1 = overhead, +1 = under feet
  // depthZ = Math.sin(skipAngle): < -0.05 is BEHIND, >= -0.05 is IN FRONT
  const elev = -Math.cos(skipAngle);
  const depthZ = Math.sin(skipAngle);
  const ropeIsBehind = depthZ < -0.08;

  // Filter pass: if isBehind does not match, return null for the cord
  if (isBehind !== ropeIsBehind) {
    // Only render handles in the front pass
    if (!isBehind) {
  ...
  // Calculate Apex Y:
  // Overhead (elev < 0): clears head by reaching up to y = -25
  // Under feet (elev >= 0): passes under feet at y = 535
  let apexY;
  if (elev < 0) {
    apexY = midY - 335 * -elev;
  } else {
  ...
  // Width flare for centrifugal expansion:
  ...
  return (
    <g id={isBehind ? "jump-rope-behind" : "jump-rope-front"}>
      {/* Floor speed shadow when rope sweeps under feet near floor */}
      {!isBehind && elev > 0.75 && (
        <ellipse
          cx={midX}
          cy="516"
      ...
      {/* Speed lines when sweeping at highest velocity under feet */}
      {!isBehind && elev > 0.82 && (
        <path
          d={`M ${leftHand.x + 10} ${leftHand.y + 10} C ${cp1X + 8} ${cp1Y}, ${cp2X} ${apexY - 14}, ${midX} ${apexY - 14} C ${cp3X} ${apexY - 14}, ${cp4X - 8} ${cp4Y}, ${rightHand.x - 10} ${rightHand.y + 10}`}
          fill="none"
          stroke="#000000"
          strokeWidth="2.5"
      ...
      {/* Handles rendered on front pass */}
