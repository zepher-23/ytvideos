...
export const TickingClock = ({
  ...
  // Second hand steps sharply every second with comic snap
  const totalSeconds = frame / 30;
  const secondStep = Math.floor(totalSeconds);
  const secondSub = (frame % 30) / 30;
  // Snap with small elastic bounce on each second tick
  ...
  return (
    <g id="ticking-wall-clock" transform={`translate(${x}, ${y})`}>
      ...
      {/* Hour Marks (12 ticks around rim) */}
      ...
      {isTickFrame && (
        <g opacity="0.85">
          ...
          />
          <path
            d="M 68 -48 Q 78 -54 84 -46"
            fill="none"
            stroke="#000000"
            strokeWidth="3"
            strokeLinecap="round"
          />
          {/* Comic text: tick / tock */}
          <text
            x="82"
            y="-68"
